...
const addMaterial = async () => {
  if (newMaterialName.value) {
    try {
      console.log("Dodavanje materijala:", newMaterialName.value);  // Dodajemo log za praćenje

      const res = await axios.post('http://localhost:3000/api/materials', {
        name: newMaterialName.value
      });

      console.log("Odgovor sa servera:", res.data);  // Logiraj odgovor sa servera

      newMaterialName.value = '';  // Resetiraj input polje nakon dodavanja
      await fetchMaterials();  // Ponovno učitaj materijale
    } catch (error) {
